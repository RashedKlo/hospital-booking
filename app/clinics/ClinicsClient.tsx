...
const ClinicsOverview = memo(({ initialClinics, initialTotalPages }: ClinicsClientProps) => {
  // State
  const [clinics, setClinics] = useState<Clinic[]>(initialClinics || []);
  const [loading, setLoading] = useState(!initialClinics);
  ...
  useEffect(() => {
    // Skip first fetch if we already have initial data for the first page
    const isFirstPageDefault = page === 1 && !filters.searchTerm && !filters.region && sortBy.sortBy === 'rating';
    if (initialClinics && isFirstPageDefault && clinics.length > 0) {
      return;
    }
    fetchClinics();
  }, [fetchClinics, initialClinics]); // Updated dependencies
